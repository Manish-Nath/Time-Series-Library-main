...
class ProbSparseAttention(nn.Module):
    ...
    def forward(self, queries, keys, values, attn_mask=None, tau=None, delta=None):
        B, L_Q, H, D = queries.shape  # Batch, Length_Query, Heads, Dim
        _, L_K, _, _ = keys.shape     # Length_Key

        # Simplified ProbSparse logic (actual implementation would involve sparsity measurement)
        scores = torch.einsum("blhd,bkhd->bhlk", queries, keys) / (D ** 0.5)
        if self.mask_flag and attn_mask is not None:
            scores.masked_fill_(attn_mask, -1e9)
        
        # Sparsity: Keep top-k scores (simplified)
